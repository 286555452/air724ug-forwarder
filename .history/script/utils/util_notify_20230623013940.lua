...
local notify = {
    ...
    -- 发送到message pusher
        ["message-pusher"] = function(msg)
            ...
            if config.MESSAGE_PUSHER_KEY == nil or config.MESSAGE_PUSHER_KEY == "" then
                log.error("util_notify", "未配置 `config.MESSAGE_PUSHER_KEY`")
                return
            end
            ...
            local body = {
                pushkey = config.MESSAGE_PUSHER_KEY or "",
                type = "text",
                text = msg
            }
    
            log.info("util_notify", "POST", config.MESSAGE_PUSHER_API)
            return util_http.fetch(nil, "POST", config.MESSAGE_PUSHER_API, header, urlencodeTab(body))
        end,
    -- 发送到 pushdeer
